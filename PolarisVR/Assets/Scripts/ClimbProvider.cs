    ...
    public class ClimbProvider : LocomotionProvider
    {
        ...
        void StepClimbMovement(ClimbInteractable currentClimbInteractable, IXRSelectInteractor currentInteractor)
        {
            ...
            if (xrOrigin != null)
            {
                ...
                var xrRigController = rigTransform.GetComponent<CharacterController>();
                if (xrRigController != null)
                {
                    xrRigController.Move(movement); // This will respect collisions
                }
                else
                {
                    rigTransform.position += movement; // fallback if no CC is attached
                }
